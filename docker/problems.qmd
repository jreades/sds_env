---
title: "Dealing with Docker Problems"
author: "Jon Reades"
date: "9/27/2024"
---
...
1. And then run this command in PowerShell or Terminal to download the SDS image with `podman pull {{< var docker.all >}}` (or `{{< var docker.mac >}}` for M-chip Macs)
...
This is a complex process, and you may need assistance, so there is a [Moving to the D Drive page](../docker/moving_to_d.qmd) to try to talk you through the process.
...
A very small number of students are **unable to run Docker or Podman at all** on computers running Windows 10 Home (in 23/24 there were *none*), in which case **Anaconda Python** can be used with the configuration [that we provide](../conda/index.qmd). However, if your machine runs Docker then [you **must** use Docker](../docker/index.qmd): this isolates the programming environment from your computer, ensuring that nothing is clobbered by accident, and guaranteeing that you are working with the same version of every Python library as the rest of the class (and the versions for which the practicals are tested). 
