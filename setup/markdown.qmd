...
Once you know where TinyTeX is, you can then follow the directions:
```bash
cd $HOME/Library/TinTeX/bin/
./universal-darwin/tlmgr option sys_bin $HOME/Library/TinyTeX/bin
./universal-darwin/tlmgr path add
```

That *should* do it, but in case you are still getting errors, then the following might be necessary:
```bash
echo 'export PATH="/Users/<your_username>/Library/TinTeX/bin:$PATH"' >> $HOME/.zshrc
```
If you don't know your username, on a Mac it's usually available by running `whoami` in the Terminal.
